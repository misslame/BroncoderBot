...
def get_announcement_message(submission_channel_id):
    ...
    if date.today().day % 3 == 0:
        ...
        message += (
            "HAY, today's MANE goals should include saddling up for today's problem."
        )
        ...
        message += (
            "I have my Exacta bet on some Broncoders in already for today's problem."
        )
    return message
...
def get_end_announcement_message(client: discord.Client, guild: discord.Guild):
    ...
    if todays_month != tomorrows_month:
        message = (
            f"The time has come!! Reign yourselves in! End of the Broncoder Derby!.\n\n"
        )
        first = ParticipantData.get_instance().get_top(1)
        if len(first) >= 1 and ParticipantData.get_instance().get_points(first[0]) > 0:
            FIRST_PLACE = guild.get_member(
                int(ParticipantData.get_instance().get_top(1)[0])
            )
            message += f"**Congrats to {FIRST_PLACE.mention}!** :tada: \n\n Here are the stats:"
            message += "\n" + format_rank_list(
                guild, ParticipantData.get_instance().get_top(10), 10
            )
        else:
